	...
	if($totalRow != 0){
		...
		while($result = mysqli_fetch_assoc($data)){
			...
			echo "<tr>
					...
					<td>
						<a href='adminUpdateMember.php?fn=$result[full_name]&fa=$result[father]&mo=$result[mother]&un=$result[user_name]&ps=$result[password]&ag=$result[age]&gn=$result[gender]&pf=$result[profession]&ed=$result[education]&pe=$result[permanent_address]&pr=$result[present_address]&ph=$result[phone_no]&bg=$result[blood_group]&sn=$sign&re=$result[religion]&we=$result[weight]&na=$result[nationality]&rd=$result[registration_date]&as=$result[account_status]'>Edit</a>
					</td>
					<td><a href='adminDeleteMember.php?id=$result[id]' onclick='return checkdelete()'>Delete</a></td>
				  </tr>";
		}
	}
	else{
