...
<?php
	...
	if($_POST['submit']){
		...
		$folder = "images/memberPicture/".$filename;
		...
		if($fn!="" && $fa!="" && $mo!="" && $un!="" && $ps!="" && $ag!="" && $gn!="" && $pf!="" && $ed!="" && $pe!="" && $pr!="" && $ph!="" && $bg!="" && $re!="" && $we!="" && $na!="" && $filename !=""){
			...
			$queryUsername = "select user_name from member where user_name='$un'";
			$queryPhone = "select phone_no from member where phone_no='$ph'";
			...
			if ($un==$resultUsername['user_name'] || $ph==$resultPhone['phone_no']) {
				...
				echo "<h3>Member Registration Failed</h3><br>";
				...
				?>
					<META HTTP-EQUIV="Refresh" CONTENT="10 URL=http://www.motivationgroupbd.org/memberRegistration.html">
				<?php
			}
				...
				$query = "insert into member(full_name, father, mother, user_name, password, age, gender, profession, education,  permanent_address, present_address, phone_no, blood_group, religion, weight, nationality, picture) values('$fn', '$fa', '$mo','$un', '$ps', '$ag', '$gn', '$pf', '$ed','$pe', '$pr', '$ph', '$bg', '$re', '$we', '$na', '$folder')";


				$data = mysqli_query($conn, $query);

				if($data){
					echo "<h3>Member Registration Successful</h3><br>";
					echo "<h3>Now You Can Log In.</h3>";

					?>
						<META HTTP-EQUIV="Refresh" CONTENT="3 URL=http://www.motivationgroupbd.org/memberLogin.php">
					<?php
					...
					?>
						<META HTTP-EQUIV="Refresh" CONTENT="3 URL=http://www.motivationgroupbd.org/memberRegistration.html">
					<?php
				}
			}
